...
quadrant main(quadrant naof_params, source_vecter params) {
  if(naof_params != 2) {
	...
	writer for_c_entree = 0;
	create_vecter(&grid, 1, 0x3d0900, &for_c_entree);
	add_string_to_sec_vecter(&grid, "for-c-entree -|- = {", 0x14, &for_c_entree);
	quad naof_pairs = 0;
		...
		while(true) {
			...
			while(true) {
				...
				while(true) {
					...
					add_string_to_sec_vecter(&grid, "\\\"", 2, &for_c_entree);
					add_string_to_sec_vecter(&grid, element[0], element[1], &for_c_entree);
					add_string_to_sec_vecter(&grid, "-", 1, &for_c_entree);
					add_string_to_sec_vecter(&grid, element2[0], element2[1], &for_c_entree);
					add_string_to_sec_vecter(&grid, "\\\"", 2, &for_c_entree);
	...
	add_string_to_sec_vecter(&grid, "}\n", 3, &for_c_entree);
	syscall(unix_write, 1, (for_c_entree + 3), for_c_entree[2]);
	return 0;
}
